...
primary outputs (`q_out`, `q31_out`) against iv_out.txt line by line.

naive_sim writes multi-bit ports bit-blasted (`$var wire 1 ! a_in[4]`), so a
vector would be reassembled MSB-first from its individual bit identifiers;
Part C's outputs are both scalars, but the helper is kept identical to Part B's
so the two comparators stay interchangeable.
"""
import sys
...
for i, expect in enumerate(ref):
    ...
    # only meaningful once the x has been shifted out of the addressed stage.
...
if bad == 0:
    print("PASS: %d cycles compared (%d skipped as x), "
          "GEM naive_sim == Icarus on q_out + q31_out" % (checked, skipped))
    sys.exit(0)
