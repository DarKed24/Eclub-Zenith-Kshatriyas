...
impl Partition {
    ...
    pub fn build_one(
        ...
        // Class C (combinational) macros: packed input lanes and staged-IO
        // scratch words, both variable-width in `chain_len`.
        let mut classc_perm_words = 0;
        let mut classc_state_words = 0;
        ...
        for &endpt_i in endpoints {
            ...
            match edg {
                ...
                EndpointGroup::Macro(m) => match m.kind.class() {
                    MacroClass::Registered => num_macros += 1,
                    MacroClass::Combinational => {
                        classc_perm_words += m.kind.num_perm_words();
                        classc_state_words += m.kind.num_state_words();
                    }
                },
        ...
        let num_output_dups = comb_outputs_activations.iter()
            ...
            .sum::<usize>();
        // A Class-R macro reserves 2 write-out words (64-bit-aligned P), plus a
        // single alignment-hole word shared by all macros in the partition. A
        // Class C macro reserves its staged-IO scratch words (bit-addressed, no
        // alignment). The packed input region borrows `num_perm_words` per
        // macro alongside the SRAM permute words. All extra terms vanish when
        // there are no macros so macro-free partitions map exactly as before.
        let num_reserved_writeouts = num_srams
            + 2 * num_macros + (num_macros != 0) as usize
            + classc_state_words
            + (num_output_dups + 31) / 32;
        if num_reserved_writeouts >= BOOMERANG_MAX_WRITEOUTS ||
            num_srams * 4 + num_macros * 4 + classc_perm_words + num_output_dups
...
/// Build one [`Partition`] per given endpoint group, WITHOUT the greedy
/// merge pass of [`process_partitions`].
///
/// Macro-heavy designs have a tiny boolean footprint, so the merge pass
/// happily fuses the whole design into ONE partition - one GPU block, one
/// SM - and every native macro evaluation is then serialised on that SM
/// while the other SMs idle (measured on `mixed_64`: 1 part vs 36 for the
/// shredded twin). Keeping the fine-grained initial cut spreads the macro
/// epilogues across blocks at the price of more global-memory traffic, which
/// for these designs is negligible (Nsight: <1% DRAM throughput).
pub fn build_partitions_no_merge(
    aig: &AIG,
    staged: &StagedAIG,
    parts: Vec<Vec<usize>>,
) -> Option<Vec<Partition>> {
    let built = parts.par_iter().enumerate().map(|(i, v)| {
