...
impl RCHyperGraph {
    ...
    ///
    /// Only available with the `partitioner` feature (which pulls in the
    /// mt-kahypar C++ dependency). Without it, feed a partition solution from
    /// a `.hgr.part` file via [`crate::pe::process_partitions_from_hgr_parts_file`]
    /// instead.
