...
fn main() {
    ...
    for cellid in 1..netlistdb.num_cells {
        if matches!(netlistdb.celltypes[cellid].as_str(),
                    "DFF" | "$__RAMGEM_SYNC_" | "GEM_DSP48E2" | "SRLC32E") {
    ...
    // SRLC32E (Part C) 32-bit shift states, by cell id.
    let mut srl_states = HashMap::<usize, u32>::new();
    ...
    for cellid in 1..netlistdb.num_cells {
        if matches!(netlistdb.celltypes[cellid].as_str(),
                    "DFF" | "$__RAMGEM_SYNC_" | "GEM_DSP48E2") {
            for pinid in netlistdb.cell2pin.iter_set(cellid) {
                if matches!(netlistdb.pinnames[pinid].1.as_str(),
                            "Q" | "PORT_R_RD_DATA" | "P") {
                    topo_vis[pinid] = true;
        ...
        if netlistdb.celltypes[cellid].as_str() == "SRLC32E" {
            srl_states.insert(cellid, 0u32);
            for pinid in netlistdb.cell2pin.iter_set(cellid) {
                // Q31 is state[31] — a pure register read, set in the latch
                // phase like a DFF Q. Q is the *combinational* addressed read
                // and stays in topo, settled in the propagate loop below.
                if netlistdb.pinnames[pinid].1.as_str() == "Q31" {
                    topo_vis[pinid] = true;
                }
            }
        }
    ...
    fn dfs_topo(netlistdb: &NetlistDB, topo_vis: &mut Vec<bool>, topo_instack: &mut Vec<bool>, topo: &mut Vec<usize>, pinid: usize) {
        ...
        if netlistdb.pindirect[pinid] == Direction::I {
            ...
            for pinid in netlistdb.cell2pin.iter_set(cellid) {
                // CARRY4 (Part B) is combinational: every output depends on all
                // of S / DI / CI / CYINIT, so recurse into them like A / B.
                // SRLC32E (Part C): only `Q` reaches here, and it depends
                // combinationally on `A` alone — `D` / `CE` are latch inputs.
                if matches!(netlistdb.pinnames[pinid].1.as_str(),
                            "A" | "B" | "S" | "DI" | "CI" | "CYINIT") {
        ...
        if matches!(netlistdb.celltypes[cellid].as_str(),
            ...
            for pinid in netlistdb.cell2pin.iter_set(cellid) {
                if matches!(netlistdb.pinnames[pinid].1.as_str(),
                            ...
                            | "A" | "B" | "C" | "OPMODE_S" | "USE_D" | "RSTP" | "CEP" | "CE") {
                    dfs_topo(&netlistdb, &mut topo_vis, &mut topo_instack, &mut topo, pinid);
                }
            }
        }
                ...
                if last_vcd_time_rising_edge {
                    ...
                    for cellid in 1..netlistdb.num_cells {
                        if netlistdb.celltypes[cellid].as_str() == "DFF" {
                            ...
                            let mut cep = 0u8;
                            let mut rstp = 0u8;
                            let mut pin_p = Vec::new();
                            for pinid in netlistdb.cell2pin.iter_set(cellid) {
                                let name = netlistdb.pinnames[pinid].1.as_str();
                                let bit = netlistdb.pinnames[pinid].2.map(|i| i as usize);
                                let canon = match name {
                                    "A" => DSP_A_OFFSET + bit.unwrap(),
                                    "D" => DSP_D_OFFSET + bit.unwrap(),
                                    "B" => DSP_B_OFFSET + bit.unwrap(),
                                    "C" => DSP_C_OFFSET + bit.unwrap(),
                                    "OPMODE_S" => DSP_OPMODE_S_OFFSET + bit.unwrap(),
                                    "USE_D" => DSP_USE_D_INDEX,
                                    "RSTP" => {
                                        if circ_state[pinid] != 0 { rstp = 1; }
                                        DSP_RSTP_INDEX
                                    },
                                    "CEP" => { cep = circ_state[pinid]; continue },
                                    "P" => { pin_p.push(pinid); continue },
                                    _ => continue,
                                };
                                if circ_state[pinid] != 0 {
                            ...
                            let mut p_cur = 0u64;
                            for &pp in &pin_p {
                                let bit = netlistdb.pinnames[pp].2.unwrap() as u64;
                                p_cur |= (circ_state[pp] as u64) << bit;
                            }
                            // en = clock-edge & (CEP | RSTP); RSTP also zeroes data
                            if cep != 0 || rstp != 0 {
                                let p_next = eval_dsp48e2(w, p_cur);
                                for &pp in &pin_p {
                                    let bit = netlistdb.pinnames[pp].2.unwrap();
                                    circ_state[pp] = ((p_next >> bit) & 1) as u8;
                                }
                            }
                        }
                        else if netlistdb.celltypes[cellid].as_str() == "SRLC32E" {
                            // Class R half of the SRL: on the rising edge, if
                            // CE == 1 the 32-bit state shifts left (LSB -> MSB)
                            // and D loads into index 0. Q31 = state[31] is a
                            // pure register read and is refreshed here; the
                            // addressed read port Q is combinational and is
                            // settled in the propagate loop below.
                            let mut d = 0u8;
                            let mut ce = 0u8;
                            let mut pin_q31 = usize::MAX;
                            for pinid in netlistdb.cell2pin.iter_set(cellid) {
                                match netlistdb.pinnames[pinid].1.as_str() {
                                    "D" => d = circ_state[pinid],
                                    "CE" => ce = circ_state[pinid],
                                    "Q31" => pin_q31 = pinid,
                                    _ => {}
                                }
                            }
                            let st = srl_states.entry(cellid).or_insert(0);
                            if ce != 0 {
                                *st = (*st << 1) | (d as u32);
                            }
                            if pin_q31 != usize::MAX {
                                circ_state[pin_q31] = ((*st >> 31) & 1) as u8;
                            }
                        }
                    }
                    // propagate
                    for &pinid in &topo {
                        ...
                        if netlistdb.pindirect[pinid] == Direction::I {
                        ...
                        else if netlistdb.celltypes[netlistdb.pin2cell[pinid]].as_str() == "CARRY4" {
                            // Class C macro golden model: settle each 4-bit
                            // slice from its pins in topo order (naive_sim sees
                            // the individual slices and their CO->CI nets, so it
                            // needs no fusion — an independent cross-check).
                            let cellid = netlistdb.pin2cell[pinid];
                            ...
                            for p in netlistdb.cell2pin.iter_set(cellid) {
                                let v = circ_state[p];
                                match (netlistdb.pinnames[p].1.as_str(), netlistdb.pinnames[p].2) {
                            ...
                            circ_state[pinid] = match name {
                                "O" => o[bit],
                                "CO" => co[bit],
                                _ => 0,
                            };
                        }
                        else if netlistdb.celltypes[netlistdb.pin2cell[pinid]].as_str() == "SRLC32E" {
                            // Class C half of the SRL: the read port outputs
                            // the bit at the dynamic address A[4:0]
                            // combinationally. Only `Q` reaches here — `Q31`
                            // was refreshed in the latch phase above.
                            let cellid = netlistdb.pin2cell[pinid];
                            let mut addr = 0u32;
                            for p in netlistdb.cell2pin.iter_set(cellid) {
                                if netlistdb.pinnames[p].1.as_str() == "A" {
                                    let i = netlistdb.pinnames[p].2.unwrap() as u32;
                                    addr |= (circ_state[p] as u32) << i;
                                }
                            }
                            let st = srl_states.get(&cellid).copied().unwrap_or(0);
                            circ_state[pinid] = ((st >> (addr & 31)) & 1) as u8;
                        }
                        else {
                            let cellid = netlistdb.pin2cell[pinid];
                            let mut vala = 0;
