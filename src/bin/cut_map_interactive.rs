...
use gem::pe::{process_partitions, build_partitions_no_merge, Partition};
...
struct SimulatorArgs {
    ...
    /// Skip the greedy partition merge pass and keep the fine-grained initial
    /// partitions, so macro-heavy designs spread over many GPU blocks (SMs)
    /// instead of collapsing into one. Pair with `--division`.
    #[clap(long)]
    no_merge: bool,
    /// Initial partition granularity in endpoints per part. The first attempt
    /// uses half of this value (GEM default 600 -> 300 endpoints/part) and
    /// halves again whenever a part exceeds the block resource limits.
...
fn main() {
    ...
    let stages_effective_parts = stageds.iter().map(|&(l, r, ref staged)| {
        ...
        let mut division = args.division;
        ...
        let effective_parts = if args.no_merge {
            clilog::info!("--no-merge: keeping the {} initial parts as-is.",
                          parts_indices_good.len());
            build_partitions_no_merge(&aig, staged, parts_indices_good).unwrap()
        } else {
            process_partitions(
