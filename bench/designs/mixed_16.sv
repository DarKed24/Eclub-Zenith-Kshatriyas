...
module mixed_16 (
  ...
  logic [16-1:0][26:0] a_pipe;  logic [16-1:0][17:0] b_pipe;
  logic [16-1:0][31:0] x_pipe, y_pipe;
  logic [47:0] p [16];
  logic [31:0] o [16], co [16], s [16];
  logic q [16];
  ...
  genvar g, c;
  generate for (g = 0; g < 16; g++) begin : lane
  ...
  always_comb begin
    pa = '0; sa = '0; qa = 1'b0;
    for (int i = 0; i < 16; i++) begin pa ^= p[i]; sa ^= o[i]; qa ^= q[i]; end
  end
  always_ff @(posedge clk) begin
