...
module carry_adders_256x64 (
  input  logic         clk, rst,
  input  logic [63:0] x_in, y_in,
  output logic [63:0] s_xor,
  output logic         c_xor
);
  logic [256-1:0][63:0] x_pipe, y_pipe;
  logic [63:0] o [256];
  logic [63:0] co [256];
  logic [63:0] s [256], di [256];
  ...
  genvar g, c;
  generate for (g = 0; g < 256; g++) begin : adder
  ...
  always_comb begin
    s_acc = '0; c_acc = 1'b0;
    for (int i = 0; i < 256; i++) begin s_acc ^= o[i]; c_acc ^= co[i][63]; end
  end
