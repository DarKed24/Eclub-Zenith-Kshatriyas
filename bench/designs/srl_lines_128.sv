...
module srl_lines_128 (
  input  logic       clk, rst, d_in, ce,
  input  logic [4:0] addr,
  output logic       q_xor, q31_xor
);
  logic [128-1:0] d_pipe;
  logic q [128], q31 [128], qc [128];
  always_ff @(posedge clk) begin
    d_pipe[0] <= d_in;
    for (int i = 1; i < 128; i++) d_pipe[i] <= d_pipe[i-1];
  end
  genvar g;
  generate for (g = 0; g < 128; g++) begin : line
  ...
  always_comb begin
    qa = 1'b0; q31a = 1'b0;
    for (int i = 0; i < 128; i++) begin qa ^= q[i] ^ qc[i]; q31a ^= q31[i]; end
  end
