...
module mixed_4 (
  ...
  logic [4-1:0][26:0] a_pipe;  logic [4-1:0][17:0] b_pipe;
  logic [4-1:0][31:0] x_pipe, y_pipe;
  logic [47:0] p [4];
  logic [31:0] o [4], co [4], s [4];
  logic q [4];
  ...
  genvar g, c;
  generate for (g = 0; g < 4; g++) begin : lane
  ...
  always_comb begin
    pa = '0; sa = '0; qa = 1'b0;
    for (int i = 0; i < 4; i++) begin pa ^= p[i]; sa ^= o[i]; qa ^= q[i]; end
  end
  always_ff @(posedge clk) begin
